...
/* We can access the useEffect() hook on the top of the React object like
   useState() hook. We can create a side effect by using useEffect()
   ...
   useEffect has one required parameter and one optional parameter:
      ...
      2. the second parameter is a dependeces array. It is optional but we 
         ...
         Already it determines when this effect will run insted of running after every single render.    */
...
// if we want to run useEffect's function depended of a verible, we need to write the verible as the second parameter    
...
export default function App() {
   ...
   /* for example here our dependence is `count` varible, it means that effect function will run on the very first time and any time `count` changes.  */
...
/* This is a common pattern when we want to set up subscriptions or event
   handlers in a component, and clean them up when the component is unmounted.   */
...
/* every component has a lifecycle
   in class components there were 3 lifecycle methods
      ...
      componnetDidAmoun()     - is invoked immediately after a component is
                                 mounted (inserted into the tree).
      componentDidUpdate()    - is invoked immediately after updating occurs
      componentWillUnmount()  - is invoked immediately before a component is
                                 unmounted and destroyed. Perform any necessary cleanup in this method, such as invalidating timers, canceling network requests, or cleaning up any subscriptions that were created in componentDidMount().
...
/* Quiz
   ...
   3. When does React run useEffect function? When does it NOT run the effect
      function?
